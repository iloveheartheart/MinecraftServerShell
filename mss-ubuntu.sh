...
if [ $USER=root ]
    then case $CHOICE1 in
         1)
             sudo apt-get install wget java >> /dev/null
             ...
             echo "正在下载服务端，请不要操作"
             wget -P $DIRECTORY https://raw.githubusercontent.com/idkwhoim/MinecraftServerArchieve/master/Jar/minecraft_server.$GAMEVERSION.jar -q
             if [ ! -e $DIRECTORY/minecraft_server.$GAMEVERSION.jar ]
                 then echo "下载失败"
                      exit
             fi
